...
def convert_identifier_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Convert the authentication window identifier to a nullable integer.
    """
    result = df.copy()
...
def convert_datetime_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Convert date and timestamp columns to consistent UTC-aware values.
    """
    result = df.copy()
...
def convert_feature_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Convert model features to numeric values and remove invalid infinities.
    """
    result = df.copy()
...
def enforce_feature_boundaries(df: pd.DataFrame) -> pd.DataFrame:
    """
    Apply logical limits and expected data types to ML features.
    """
    result = df.copy()
...
def remove_invalid_rows(df: pd.DataFrame) -> pd.DataFrame:
    """
    Remove rows that cannot be safely scored or persisted.
    """
    result = df.copy()
...
def remove_duplicate_windows(df: pd.DataFrame) -> pd.DataFrame:
    """
    Keep only the most recent record for each authentication window.
    """
    result = df.copy()
...
def validate_training_dataset(df: pd.DataFrame) -> None:
    """
    Ensure the resulting dataset is large enough for model training.
    """
    if df.empty:
        raise ValueError(
...
def prepare_authentication_features(
    df: pd.DataFrame,
) -> pd.DataFrame:
    """
    Validate and normalize authentication features loaded from PostgreSQL.
    """
    if df.empty:
        raise ValueError(
...
def load_data() -> pd.DataFrame:
    """
    Load and prepare authentication ML features from PostgreSQL.
    """
