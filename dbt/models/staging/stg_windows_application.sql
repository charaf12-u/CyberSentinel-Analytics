...
with source_data as (
    ...
    select *
    from {{ source('bronze', 'windows_application_logs') }}
...
typed as (
    ...
    select
        ...
        {{ normalize_null('application_name') }}
            as application_name,

        {{ normalize_null('faulting_application') }}
            as faulting_application,

        {{ normalize_null('faulting_module') }}
            as faulting_module,

        {{ normalize_null('exception_code') }}
            as exception_code,
        ...
        {{ normalize_null('service_name') }}
            as service_name,

        {{ normalize_null('error_bucket') }}
            as error_bucket,

        {{ normalize_null('user_sid') }}
            as user_sid,
